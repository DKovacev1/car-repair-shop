package hr.autorepair.common.constants;

public class MessageConstants {
    private MessageConstants(){}
    public static final String CODE_ERROR_MESSAGE = "Verification code has expired!";
    public static final String WRONG_CREDENTIALS = "Wrong credentials!";
    public static final String LOGIN_NOT_ALLOWED = "Signing in is allowed only after admin accepts your sign up request!";
    public static final String EMAIL_ALREADY_IN_USE = "User with e-mail {0} is already using the application!";
    public static final String ROLE_NAME_NOT_EXIST = "There is no role with name {0}!";
    public static final String ROLE_ID_NOT_EXIST = "There is no role with id {0}!";
    public static final String EMAIL_NOT_EXIST = "Failed to send email!";
    public static final String USER_NOT_EXIST = "There is no user with id {0}!";
    public static final String NO_CHANGES_MADE = "No changes were made!";
    public static final String WORKSHOP_NOT_EXISTS = "There is no workshop with id {0}!";
    public static final String REPAIR_NOT_EXISTS = "There is no repair with id {0}!";
    public static final String CAR_NOT_EXISTS = "There is no car with id {0}!";
    public static final String JOB_ORDER_NOT_EXISTS = "There is no job order with id {0}!";
    public static final String JOB_ORDER_STATUS_NOT_EXISTS = "There is no job order status with id {0}!";
    public static final String SCHEDULE_NOT_FREE = "There is no free workplace for given date and time!";
    public static final String RECEIPT_NOT_EXIST = "There is no receipt with id {0}!";
    public static final String RECEIPT_NOT_EXIST_FOR_JOB_ORDER = "There is no receipt for job order with id {0}!";
    public static final String PAYMENT_NOT_EXIST = "There is no payment with id {0}!";
    public static final String JOB_ORDER_NOT_FINISHED = "Job order is not finished!";
    public static final String RECEIPT_ALREADY_CREATED = "Receipt already created.";
    public static final String PART_NOT_EXIST = "There is no part with id {0}!";
}
